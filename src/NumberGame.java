						...
						case 2:
							...
							while ((usrScores[0] >= 0 && usrScores[0] <= 100)
								...
								Random compNum = new Random(testSeed); // FIXME: take out (testSeed) when game is done
																		// testing
								compScores[0] = compNum.nextInt(101);
								compScores[1] = compNum.nextInt(101);
								compScores[2] = compNum.nextInt(101);

								// The computer numbers are printed for testing purposes
								System.out.println(compScores[0]);
								System.out.println(compScores[1]);
								System.out.println(compScores[2]);// FIXME get rid of this after testing
								...
								if ((usrScores[0] <= 100) && (usrScores[0] >= 0)) {
									...
									int bigger = Math.max(compScores[0], usrScores[0]);
									int smaller = Math.min(compScores[0], usrScores[0]);
									...
									if ((bigger == usrScores[0]) && !((usrScores[0] - compScores[0]) == 69)
											&& (usrScores[0] != compScores[0])) {
										usrScore = usrScore + 5;
									} else if ((bigger == compScores[0]) && !((usrScores[0] - compScores[0]) == 69)
											&& (usrScores[0] != compScores[0])) {
									...
									if ((usrScores[0] == bigger) && (usrScores[0] - compScores[0]) <= 20) {
										usrScore = usrScore - 8;
									} else if ((usrScores[0] == smaller) && (compScores[0]) <= 20) {
										compScore = compScore - 8;
									}

									// Takes 1 point from smaller number
									if (usrScores[0] == smaller && (compScores[0] - usrScores[0]) != 5) {
										usrScore = usrScore - 1;
									} else if (compScore == smaller && (usrScores[0] - compScores[0]) != 5) {
										compScore = compScore - 1;

										// If smaller number is five or less smaller than the large number, the player
										// with the small number gets 6 points
										if (usrScores[0] == bigger && (usrScores[0] - compScores[0]) == 5) {
											compScore = compScore + 6;
										} else if (compScores[0] == bigger && (compScores[0] - usrScores[0]) == 5) {
											usrScore = usrScore + 6;
										}
